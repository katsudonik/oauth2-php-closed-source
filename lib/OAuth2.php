...
class OAuth2 {
	...
	public function verifyAccessToken($token_param, $scope = NULL) {
		...
		// add katsudonik -->>
		// check client (for token replace attack to client, if check client by auth server. normal: use debug_token api by client and check client by client)
...
// 		}
		...
		// add katsudonik -->>
// 		// Check token's client (for token replace attack to client, if check client by auth server. normal: use debug_token api by client and check client by client)
